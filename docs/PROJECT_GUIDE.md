...
14. [Core concepts reference](#14-core-concepts-reference)
...
| `requirements/base.txt` / `requirements/notebooks.txt` **[E/O]** | Core app deps vs the notebook/analysis extras (pandas, matplotlib, seaborn). | Manual install still works. |
...
> An earlier structured **XGBoost/DDXPlus** classifier once lived here; it was
> **removed** for a train/serve mismatch (see §5's design note). The live ML is now
> the single free-text model above.
...
Unit/integration tests (204 passing); setup/healthcheck/calibration scripts; the
two data-science notebooks (`text_classifier_analysis` + `rag_corpus_analysis`).
...
**One live model — the free-text classifier.** An earlier structured XGBoost
classifier was evaluated and **removed**; §5b explains why (it's the project's
headline distribution-shift lesson).
...
**Why the live ML is free-text (the key story):** **train == serve.** It's trained
and served on the *same* distribution (free-text symptoms), so its held-out score
...
### 5b. Design note — why not a structured classifier (the XGBoost lesson)

An earlier version of the live ML was a **DDXPlus XGBoost** classifier: it encoded
a patient into a ~229-d vector (multi-hot symptom codes + age bins + sex) and
predicted a disease from complete evidence vectors.

**Why it was removed — train/serve mismatch.** It was trained on *complete*
evidence (all symptoms known), but at serve time a chat message yields only a
handful of parsed symptoms with everything else marked "absent" — a vector the
model never saw in training → out-of-distribution → confidently wrong (flu →
"TB 91%"). You *can't* reconstruct a full structured vector from a sentence, so
the mismatch is unfixable in that role. It scored well on its *own* test set, which
is exactly the point: **a great test score is useless if production data comes from
a different distribution.** That lesson drove the switch to the free-text model in
§5a (train == serve), and the XGBoost code was dropped from the repo once it had
made its point.
...
**Runtime:** Ollama (local, free) over HTTP. Default `llama3.1:8b` (the more
faithful choice, benchmarked; also the eval judge); `qwen3:1.7b` is the snappier
CPU-demo alternative. `num_predict=768` so a full differential isn't truncated.
...
- **Q: Walk me through why you replaced the XGBoost model.** *A:* Train/serve
  mismatch. XGBoost trained on complete structured symptom vectors, but a chat
  message only yields a few parsed symptoms with the rest marked absent —
  out-of-distribution, so it was confidently wrong. I replaced the live model with
  one that trains and serves on the same thing (free text embedded with the
  retriever's encoder), so its held-out 0.915 top-1 actually transfers. The XGBoost
  model was good on *its own* distribution — which is precisely why it's such a
  clean illustration of distribution shift — but it had no place on the live path,
  so it was removed.
...
## 14. Core concepts reference

The concepts the project *uses* — worth being able to explain from first
principles. RAG flow, FastAPI, Streamlit state, the ML pipeline shape, and why
train/serve mismatch matters are covered above; the list below is the deeper
theory each component rests on:
...
8. **Distribution shift / OOD** — the single most important theoretical idea in the
   project (the XGBoost/train-serve-mismatch story). Covariate shift vs concept shift.
...
**How to use it:** the highest-leverage topics are **#8 (distribution shift)** and
**#4 (bi/cross-encoder)** — those two ideas anchor the project's strongest design
stories.
